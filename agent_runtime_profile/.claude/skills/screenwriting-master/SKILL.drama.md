...
description: "ArcReel 适配版全格式编剧工作流。用于故事概念、短片/长片/剧集大纲、人物弧光、场景拆解、剧本医生，以及把编剧方案落回当前剧集动画项目。当用户提到写剧本、想故事、编剧、大纲、人物设计、节拍表、场景拆解、剧本医生、短片、电影、剧集、what-if 或 how-to-tell 时使用。"
---
<!-- mode: drama -->

# 山音超级编剧大师（ArcReel 剧集动画适配版）
...
- ArcReel 项目内：优先服务 `content_mode == "drama"` 的剧集动画项目，补强前期创作、规范化剧本、场景拆解和对白动作改写
...
2. 目标集文件：优先读取用户指定的 `source/episode_N.txt`、`drafts/episode_N/step1_normalized_script.md` 或 `scripts/episode_N.json`
3. 若用户未指定集数，只做全局创作咨询；不要猜测要修改哪一集

剧集动画项目的关键边界：

- 规范化剧本落点优先是 `drafts/episode_N/step1_normalized_script.md`
- 编剧方案和诊断可保存到 `drafts/episode_N/screenwriting_plan.md`
- 已有 JSON 剧本的可影响生成字段优先是 `visual`、`dialogue`、`action`、`image_prompt`、`video_prompt`、`transition_to_next`、`note`
- 不随意改 `scene_id`、`duration_seconds`、`generated_assets`，不凭空增删角色/场景/道具
...
ArcReel 剧集动画项目中，分集建议必须能对应 `source/episode_N.txt` 的自然断点，不要只按字数硬切。
...
`场景 | 叙事功能 | 角色目标 | 阻碍 | 动作设计 | 对白策略 | 画面焦点 | 转场策略 | 生成提示`

要求：

- 每场戏都要有明确的目标与阻碍
- 对白必须口语化，避免解释设定和主题
- 动作与反应要能支撑动画表演，而不是只写情绪标签
- 如果用户要存档，可保存到 `drafts/episode_N/screenwriting_plan.md`

### 第 5 步：场景写作与视听化

在用户确认拆解后，再写具体场景文本。剧集动画模式中：

- 首次生成或大改规范化剧本时，优先落到 `drafts/episode_N/step1_normalized_script.md`
- 新写内容必须有可演的动作、可听见的对白、可拍的画面，不写心理说明
- 对白要口语化，避免解释设定和主题
- 每完成一批场景后暂停，等待用户“通过 / 修改 / 自检 / 继续”

### 第 6 步：剧本医生与落回项目

诊断已有 `scripts/episode_N.json` 时，按以下顺序：

1. 结构问题：场景目标、阻碍、转折是否清晰
2. 表演问题：动作、对白、反应是否可演
3. 视听问题：是否有无法拍摄的抽象描述
4. 生成问题：`image_prompt` / `video_prompt` 是否过泛、过空或互相矛盾
5. 长内容连续性：伏笔、回扣、角色状态、世界观信息释放是否前后一致

用户确认“落回项目”后，只修改已确认范围；完成后总结改了哪些场景、哪些字段、预期影响。
...
- 是否让每场戏具备目标、阻碍、动作和可拍的视听表达
- 是否为长片/剧集维护了前史、世界观、伏笔、Subplot 和检查点
- 是否把抽象判断转化为可演的动作、对白、画面或结构调整
